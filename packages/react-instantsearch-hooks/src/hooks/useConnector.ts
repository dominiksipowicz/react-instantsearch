    ...
    const createWidget = connector(
      (connectorState, isFirstRender) => {
        ...
        if (shouldSetStateRef.current) {
          ...
          // We base the check on non-function parameters only because functions
          // cannot be compared. It's safe to omit them because functions like
          // `refine()` and `createURL()` are stable in InstantSearch.js.
